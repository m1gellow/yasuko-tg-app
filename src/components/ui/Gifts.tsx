  ...
  return (
    <div className={`bg-[#252538] p-3 rounded-lg ${unlocked ? 'border border-yellow-500' : ''}`}>
      <div className="flex items-start">
        <div
          className={`rounded-full flex items-center justify-center w-10 h-10 mr-3 ${unlocked ? 'bg-yellow-500 text-black' : 'bg-[#323248] text-gray-400'}`}
        >
          {number}
        </div>
        <div className="flex-grow">
          <div className="flex justify-between">
            ...
            {unlocked ? (
              <button onClick={onClaim} className="bg-green-500 text-white px-3 py-1 rounded text-sm">
                Получить
              </button>
            ) : (
    ...
    <div className="bg-[#252538] rounded-lg p-4 mb-4">
      ...
      <div className="mb-2">
        ...
        <div className="w-full bg-[#323248] h-2 rounded-full overflow-hidden">
          <div className="bg-yellow-500 h-full" style={{ width: `${(progress / maxProgress) * 100}%` }} />
        </div>
      </div>
...
const TaskItem: React.FC<TaskItemProps> = ({
  ...
  return (
    <div className="bg-[#252538] p-3 rounded-lg mb-3">
      ...
      {completed && (
        <button onClick={onComplete} className="w-full bg-green-500 text-white py-1 rounded font-medium text-sm">
  ...
  return (
    <div className={`bg-[#252538] p-3 rounded-lg mb-3 ${isUnlocked ? 'border border-yellow-500' : ''}`}>
      <div className="flex items-center">
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${isUnlocked ? 'bg-yellow-500 text-black' : 'bg-[#323248] text-gray-400'}`}
        >
          {level}
        </div>
...
const Gifts: React.FC = () => {
  ...
  return (
    <div className="bg-[#1E1E2D] min-h-screen p-4 pb-20 text-white">
      ...
      {activeTab === 'seasons' && (
        <div>
          <SeasonSection season="ВЕСЕННИЙ СЕЗОН" daysLeft={31} progress={150} maxProgress={1200} />
          ...
          <div className="bg-yellow-500 text-black p-4 rounded-lg mb-4">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm">Развивайте вашего персонажа, чтобы получать награды за уровни</p>
              </div>
            </div>
          </div>
