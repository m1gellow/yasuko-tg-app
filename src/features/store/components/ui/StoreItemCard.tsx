      ...
      <div className="p-3">
        <div className="flex justify-between items-center mb-2">
          ...
          {item.discountPercent && (
            <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded">-{item.discountPercent}%</span>
          )}
          {item.isNew && <span className="bg-blue-500 text-white text-xs px-2 py-0.5 rounded">НОВИНКА</span>}
        </div>
        ...
        <div className="flex">
          ...
          <div className="flex-grow">
            ...
            <div className="flex items-center justify-between mt-2">
              <div>
                {item.originalPrice && (
                  <span className="text-gray-400 line-through text-sm mr-2">{item.originalPrice} МОН</span>
                )}
              ...
              <button
                className={`px-4 py-1 rounded font-medium ${
                  userCoins >= item.price ? 'bg-blue-500 hover:bg-blue-600' : 'bg-gray-600 cursor-not-allowed'
                }`}
