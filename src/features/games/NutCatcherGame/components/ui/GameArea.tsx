...
import { FlameIcon, StarIcon, ClockIcon } from 'lucide-react';
import { GameAreaProps, NutType } from '../../../../../types';
...
const GameArea: React.FC<GameAreaProps> = ({ isActive, onScoreUpdate, onTimerUpdate }) => {
  const [nuts, setNuts] = useState<NutType[]>([]);
    ...
    const gameLoop = (timestamp: number) => {
      ...
      if (timestamp - lastNutTime.current > 800 - Math.min(400, score * 10)) {
        ...
        if (squirrel.throwing) {
          ...
          const nutType = nutTypeRand < 0.7 ? 'acorn' : nutTypeRand < 0.85 ? 'walnut' : 'hazelnut';

          const newNut: NutType = {
      ...
      setNuts((prevNuts) => {
        const updatedNuts = prevNuts
          ...
          .filter((nut): nut is NutType => nut !== null);
      ...
      >
        ...
        {nutCatchAnimation.active && (
          <NutCatchAnimation
            ...
            points={(NUT_POINTS[nutCatchAnimation.type as 'acorn' | 'walnut' | 'hazelnut'] * comboMultiplier).toFixed(
              0,
            )}
          />
        )}
        ...
        {/* Сообщение о комбо */}
        {showComboMessage && <ComboMessage message={comboMessage} />}
      </div>
