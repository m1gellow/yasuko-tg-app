...
const CompetitionCard: React.FC<CompetitionCardProps> = ({
  ...
  const hasAchievedPosition = tournamentPosition.position !== null && tournamentPosition.position <= requiredPosition;
  ...
  return (
    <div className="bg-[#252538] flex flex-col gap-[1rem] items-center justify-center rounded-lg p-4 mb-4">
      ...
      <h4 className="font-medium text-xl">
        Главный приз <span className="text-yellow-500">{prizePool} ₽</span>
      </h4>
      <p className="text-[white] font-light text-xs uppercase text-center">
        правила очень простые! Для участия в розыгрыше нужно занять лидирующую поизицию в рейтинге с 1 по 20 место.
        Победитель будет выбран слачайно среди первых 20 игроков!
      </p>
      ...
      <div className="space-y-2">
        ...
        <button
          className={`w-full ${
            isLoading || !user
              ? 'bg-gray-600'
              : hasAchievedPosition && canClaimReward
                ? 'bg-green-500'
                : 'bg-yellow-500'
          } text-black py-2 rounded-lg font-medium mt-2`}
          disabled={isLoading || !user || (hasAchievedPosition && !canClaimReward)}
