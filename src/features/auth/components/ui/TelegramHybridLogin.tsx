...
const TelegramHybridLogin: React.FC<TelegramHybridLoginProps> = ({ onSuccess, onError }) => {
  ...
  const { signUpOrSignInWithTelegram, isLoading, error, authResult, resetError } = useTelegramHybridAuth();
    ...
    <div className="w-full">
      {isReady ? (
        <div className="flex flex-col gap-3">
          ...
          {authResult?.success && (
            <div
              className={`mt-2 p-3 rounded-lg ${authResult.isNewUser ? 'bg-blue-500/20 border border-blue-500' : 'bg-green-500/20 border border-green-500'}`}
            >
              <div className="flex items-start">
                <CheckCircle size={18} className="mr-2 mt-0.5 flex-shrink-0 text-green-400" />
                <p className="text-green-300">{authResult.isNewUser ? 'Регистрация' : 'Вход'} выполнен успешно!</p>
              </div>
            </div>
          )}
            ...
            <div className="mt-4 p-4 bg-blue-500/20 border border-blue-500 rounded-lg">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex">
                    <div className="bg-[#323248] p-2 rounded flex-grow text-white">{authResult.email}</div>
                    <button
                      onClick={copyEmail}
              ...
              <div className="mt-4">
                ...
                <button className="w-full mt-2 py-2 rounded-md bg-green-600 text-white" onClick={handleManualClose}>
