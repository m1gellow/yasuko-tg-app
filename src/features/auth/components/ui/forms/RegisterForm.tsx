...
import { LucideUser, AlertTriangleIcon, CheckIcon, Loader2Icon, Eye, EyeOff, Copy } from 'lucide-react';
...
const RegisterForm: React.FC<RegisterFormProps> = ({ onSwitchToLogin, onRegistrationSuccess, telegramData }) => {
  const { loading: authLoading } = useAuth();
  const { user: telegramUser } = useTelegram();
  const { signUpWithEmail, isLoading, error: emailAuthError, success, resetError } = useEmailAuth();
    ...
    <div className="bg-[#252538] rounded-lg p-6 w-full max-w-md mx-auto">
      ...
      {usingTelegram && (
        <div className="mb-4 p-3 bg-[#2AABEE]/20 border border-[#2AABEE] rounded-md flex items-center">
          {avatarUrl ? (
            <img src={avatarUrl} alt="Telegram Avatar" className="w-10 h-10 rounded-full mr-3" />
          ) : (
            <LucideUser size={24} className="text-[#2AABEE] mr-3" />
          )}
          <div>
            <p className="text-sm text-white">Регистрация с данными из Telegram</p>
